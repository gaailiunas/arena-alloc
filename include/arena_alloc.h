...
#ifndef ARENA_DEFAULT_LOAD_FACTOR
...
#define ARENA_ALLOC(...)                                                       \
    ARENA_ALLOC_GET_MACRO(__VA_ARGS__, ARENA_ALLOC_3, ARENA_ALLOC_2,           \
                          ARENA_ALLOC_2)(__VA_ARGS__)
...
struct mem_arena {
    ...
    size_t offset;
    uint8_t flags;
#ifdef ARENA_DYNAMIC_LOAD_FACTOR
    double load_factor;
#endif
};
 ...
 */
static inline int arena_init(struct mem_arena *arena, char *mem,
                             size_t initial_sz, uint8_t flags)
{
    if (!mem) {
    ...
    arena->offset = 0;
#ifdef ARENA_DYNAMIC_LOAD_FACTOR
    arena->load_factor = ARENA_DEFAULT_LOAD_FACTOR;
#endif
    return 0;
}
...
class Arena {
  public:
    /* `initial_sz` in bytes */
    explicit Arena(const std::size_t initial_sz, const std::uint8_t flags = 0)
        : _dtor_head(nullptr)
    {
    ...
    /* `initial_sz` in bytes */
    explicit Arena(char *mem, const std::size_t initial_sz,
    ...
    mem_arena *get() { return &_arena; }

    template <typename T, std::size_t N = 1,
              std::size_t alignment = ARENA_ALIGNOF(T), typename... Args>
    T *alloc(Args &&...args)
    {
        ...
        if constexpr (!std::is_trivially_destructible_v<T>) {
            DestructorNode *node = reinterpret_cast<DestructorNode *>(
                arena_alloc(&this->_arena, sizeof(DestructorNode),
                            alignof(DestructorNode)));
            if (!node)
                return nullptr;
    ...
    void *alloc_raw(std::size_t nbytes,
                    std::size_t alignment = ARENA_DEFAULT_ALIGNMENT)
    {
...
template <typename T> class ArenaAllocator {
  public:
    ...
    /*
     * since stl containers track references of objects, that becomes a massive
     * overhead.
     */
    void deallocate(T *p, size_type n) noexcept
    {
        // this is an arena allocator and we don't have to deallocate every
        // object individually
    }
